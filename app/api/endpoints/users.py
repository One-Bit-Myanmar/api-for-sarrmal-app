...
async def delete_user(
    ...
    user = user_collection.find_one({"user_id": user_id})
    if user: # if exist
        user_collection.delete_one({"user_id": user_id}) # delete that account
        session_collection.delete_one({"user_id": user_id}) # delete the session
...
async def delete_user(
    ...
    if current_user: # if exist
        user_collection.delete_one({"user_id": current_user["user_id"]}) # delete that account
        session_collection.delete_one({"user_id": current_user["user_id"]}) # delete the session
        return {"response": "success", "message": "User deleted Successfully"} # repsone that the account was deleted successfully
    else:
        # else we will raise an error for that mistake
        raise HTTPException(status_code=404, detail="User not found")
...
async def logout(  
    ...
    user = user_collection.find_one({"user_id": current_user["user_id"]}) # user key["value"] as it is a dict
    if user:
        session_collection.delete_one({"user_id": current_user["user_id"]})
